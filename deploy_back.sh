...
# 4.1.1 : Nettoyage du commit temporaire de .env
  if [ "$ENV_TEMP_COMMIT" = true ]; then
    echo "[INFO] Suppression du commit temporaire de .env (reset --soft HEAD~1)..."
    git reset --soft HEAD~1 || echo "[WARNING] Échec du reset. Supprimez manuellement le commit si nécessaire."
  fi
