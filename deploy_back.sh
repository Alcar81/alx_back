...
# 3.1 Sauvegarde du contenu actuel de production
  echo "[INFO 3.1.1] Vérification et création du répertoire de sauvegarde si nécessaire... $(date)" | tee -a "$LOG_FILE"
  if [ ! -d "$BACKUP_DIR" ]; then
    echo "[INFO] Création du répertoire de sauvegarde : $BACKUP_DIR"
    mkdir -p "$REPO_PROD" || error_exit "Échec de la création du répertoire $REPO_PROD."
  fi

  if [ -d "$REPO_PROD" ]; then
    echo "[INFO 3.1.2] Sauvegarde du contenu actuel de $REPO_PROD dans $BACKUP_FILE..."
    tar -czf "$BACKUP_FILE" -C "$REPO_PROD" . || error_exit "Échec de la sauvegarde du contenu de production."
    echo "[SUCCESS] Sauvegarde réalisée avec succès : $BACKUP_FILE"

    echo "[INFO 3.1.3] Rotation des sauvegardes pour limiter à $MAX_BACKUPS fichiers..."
    BACKUP_COUNT=$(ls -1 "$BACKUP_DIR"/*.tar.gz 2>/dev/null | wc -l)
    if [ "$BACKUP_COUNT" -gt "$MAX_BACKUPS" ]; then
      echo "[INFO 3.1.4] Suppression des sauvegardes les plus anciennes..."
      ls -1t "$BACKUP_DIR"/*.tar.gz | tail -n +$((MAX_BACKUPS + 1)) | xargs rm -f || error_exit "Échec de la rotation des sauvegardes."
      echo "[SUCCESS] Rotation des sauvegardes effectuée avec succès."
    fi
  else
    echo "[INFO] Le répertoire $REPO_PROD n'existe pas. Aucune sauvegarde nécessaire."
  fi
